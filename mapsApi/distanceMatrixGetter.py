...
for idx1 in range(0, fromToMatrix.index.size):
    ...
    for idx2 in range(0, fromToMatrix.index.size):
        ...
        if idx2 % 23 == 0 and idx2 > 0:
            # Make the API request
            matrix = gmaps.distance_matrix(originString, destString[:-1], units="metric")  # [:-1] is to remove last "|"
            ...
            for idx in range(0, newmat.index.size):
                fromToMatrix.iloc[idx1, startVal + idx] = newmat["distance"].iloc[idx]["value"]
