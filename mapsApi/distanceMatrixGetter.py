...
def makeAPIcall(origin, dest):
    ...
    if newmat['status'][0] == "MAX_ELEMENTS_EXCEEDED ":
        logging.warn("Max elements exceeded")
        return (pd.DataFrame(), False)
    if newmat['status'].unique().size > 1 or newmat['status'][0] != "OK":
        logging.error("Query to mapping API failed with error " + newmat['status'][0])
        quit()
    # Verify that it's the right size
    elif newmat['distance'].size != len(destString[:-1].split("|")) * len(originString.split("|")):
...
apiMatrix = []
# Need a loop to create the origin/destination strings for t he API
